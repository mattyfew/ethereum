...
function    doGetNodeStatus()  {
    ...
    web3.eth.getSyncing(function(err, result) {
        if(err) setData('mf_problem_in_getSyncing', error, true)

        document.querySelector('#node_syncing_status .node_result').innerHTML = result
    })

    web3.eth.getMining(function(err, result) {
        if(err) setData('mf_problem_in_getMining', error, true)

        document.querySelector('#node_mining_status .node_result').innerHTML = result
    })
